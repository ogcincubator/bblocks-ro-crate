...
# RO-Crate JSON-LD Context (Model)

`ogc.researchobject.ro-crate.context` *v1.0.0*
...
description: 'Unconstrained carrier schema: this block''s content is its assembled
  JSON-LD context, not a data shape. It aggregates the RO-Crate vocabulary building
  blocks via `allOf`/`$ref` purely so the postprocessor''s context-assembly mechanism
  (which is driven by schema `$ref` resolution) combines all of their JSON-LD contexts
...
* [RO-Crate 1.3 Metadata section (vocabulary provenance)](https://www.researchobject.org/ro-crate/specification/1.3/metadata.html)
